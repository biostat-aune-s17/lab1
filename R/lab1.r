...
# returns the second column of matrix demo.matrix
...
AMRO
rather than ‘birds$AMRO.’ This is more concise notation, but it means that we cannot have a variable with the same name in another attached data frame, so let’s detach it:
detach(birds)

5.3 Lists
As noted above, a list is a compound object composed of associated data. Items within a list are generally referred to as components. Similar to data frames, components in a list can be given a name, and the component can be specified by name at any time. In addition, components can be specified by their position in the list, similar to a subscript in a vector. However, in contrast to a vector, lists components are specified in double [[ ]] delimiters. We may ultimately find it handy to create our own lists, but for the first few labs we will just see them as results from analyses, so we'll take them as they come and demonstrate their properties by example.
For the time being, here’s a very simple example. Using the demo.vector1 vector above, and the names of the birds data frame.
...
One way to view the structure of an object is via the str() function. For example, let’s once again view the structure of the birds data frame as follows:
str(birds)
Note, the result tells us that the object is a data frame containing 32 observations and 10 variables.
Furthermore, it lists each variable (columns), in order, the associated class or data type (e.g., factor or integer in this case), and a sample of the first 10 values. For factors, it also gives the number of levels (i.e., unique values) and what they are. This information can be incredibly useful in determining why some operation doesn’t work, because it usually has to do with having an ineligible data type.
For large data sets, it is sometimes useful to view a portion of the object to make sure it looks right.
Two functions, head() and tail() will print out the first and last 6 six records of the object, respectively, although you change the number of records to print by adding an argument to the function. For example, we can print out the first 10 records of the birds data frame as follows:
head(birds,10)
...
| for ‘or’
For example,
sum(birds[,5]>1) --- calculates the number of plots where the abundance of the species in column 5 is greater than 1 (birds[,5]>1 is evaluated as 1 (true) or 0 (false), so that the sum is of 0's and 1's).
sum(birds[,5][birds[,5]>1]) --- calculates the sum of the abundance for the species in column 5 in plots where species in column 5 has abundance greater than 1.
max(birds[,5][birds$BHGR==5]) --- calculates the maximum abundance for the species in column 5 for plots with the abundance of BHGR equal to 5.

6.1 Missing Values
A final special case is of special note. Missing values in a vector or matrix are always a problem in ecological data sets. Sometimes it is best simply to remove samples with missing data, but often only one or a few values are missing, and it's best to keep the sample in the matrix with a suitable missing value code. We'll discuss how to deal with missing values in more detail in the future, but for now let’s assume that we want to drop the missing values from a vector. First, select the fourth column from the birds data frame, which contains a single missing value (note we are assigning that vector to a new object called “x”):
x<-birds[,4]
...
is.na( ) # note that this will fail, since we aren’t including an argument
so, to include all of a vector except missing values, we set a logical test to be true when values are not missing. Since the R operator for ‘not’ is !, the correct test is:
!is.na( ) #again, it needs an argument
and to specify which vector we're testing for missing value, we put the vector in parentheses as follows:
!is.na(x) # this will print whether the logical test is true or false for each value in x
...
x[x!=’NA’]
will NOT work. Since dropping missing records is a very common operation, R has a built in function, na.omit(), that makes it even easier to accomplish the same thing as above. Try the following:
na.omit(x)
...
The first argument, x, is the data set (either a vector, matrix, or data frame containing all numeric variables) you wish to sum, and the second argument indicates whether missing values should be ignored. The default na.rm=FALSE will return NA if there are any missing values, whereas
